...
function getModel() {
    ...
    model = tf.sequential();
    model.add(tf.layers.conv2d({inputShape: [28, 28, 1], kernelSize: 3, filters: 32, stride:'same', activation: 'relu', kernel_initializer: 'he_uniform'}));
    model.add(tf.layers.maxPooling2d({poolSize: [2, 2]}));
    model.add(tf.layers.conv2d({kernelSize: 3, filters: 64, stride:'same', activation: 'relu', kernel_initializer: 'he_uniform'}));
    model.add(tf.layers.maxPooling2d({poolSize: [2, 2]}));
    model.add(tf.layers.conv2d({kernelSize: 3, filters: 128, stride:'same', activation: 'relu', kernel_initializer: 'he_uniform'}));
    model.add(tf.layers.maxPooling2d({poolSize: [2, 2]}));
